...
export function PdfAdmin({ initial, cache = { version: 1, targets: {} }, persistence = localPersistence, enabled = false, generatePdf }: { initial: ContentData; cache?: PdfCacheManifest; persistence?: AdminPersistence; enabled?: boolean; generatePdf?: PdfGenerator }) {
  ...
  const action = (target: PdfTarget, label: string, tone: "primary" | "secondary" = "primary") => <PdfGenerateAction generatePdf={generatePdf} target={target} label={label} sourceHash={pdfSourceHash(content, target)} cacheEntry={cache.targets[target]} disabled={!enabled} tone={tone} />;
  ...
  return <main className="adminPage pdfAdmin">
    <section className="adminTitle"><div><p>Content / PDF</p><h1>PDF 生成</h1></div><Link href="/admin">返回案例管理</Link></section>
