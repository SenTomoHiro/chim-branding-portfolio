...
export function PdfAdmin({ initial, persistence = localPersistence, enabled = false }: { initial: ContentData; persistence?: AdminPersistence; enabled?: boolean }) {
  ...
  return <main className="adminPage pdfAdmin"><section className="adminTitle"><div><p>Content / PDF</p><h1>PDF 生成</h1></div><Link href="/admin">返回案例管理</Link></section>
    ...
    <section className="formSection pdfAdminSection"><div className="sectionHeading"><div><h2>合集生成</h2><p>按当前已保存的合集成员和精选图片生成。</p></div></div><div className="pdfAdminActions"><PdfGenerateAction target="design" label="生成 Design Portfolio" disabled={!enabled} /><PdfGenerateAction target="photography" label="生成 Photography Portfolio" disabled={!enabled} /></div></section>
    <section className="formSection pdfAdminSection"><div className="sectionHeading"><div><h2>设计分类生成</h2><p>输出文件名固定，可重复覆盖本地结果。</p></div></div><div className="pdfCategoryGrid">{CASE_CATEGORIES.map((entry) => <article key={entry.value}><div><strong>{entry.zh}</strong><span>portfolio-design-{entry.value}.pdf</span></div><PdfGenerateAction target={`category:${entry.value}`} label={`生成${entry.zh}合集`} disabled={!enabled} tone="secondary" /></article>)}</div></section>
    <section className="formSection pdfAdminSection"><div className="sectionHeading"><div><h2>案例与合集成员</h2><p>成员开关直接保存到案例；单案例 PDF 不受合集成员状态影响。</p></div><span>{visible.length} / {cases.length}</span></div>
      ...
      <div className="pdfCaseManagement">{visible.map((item) => <article key={item.id}><div><strong>{caseFullTitle(item)}</strong><span>{item.business === "branding" ? "Design" : "Photography"}{item.categories.length ? ` · ${categoryLabels(item.categories)}` : ""}{!item.published ? " · 草稿" : ""}</span></div><label className="checkLabel"><input type="checkbox" checked={item.includeInPortfolioPdf} disabled={pendingIds.includes(item.id)} onChange={() => void toggleMembership(item)} /> 加入合集</label><PdfGenerateAction target={`case:${item.id}`} label="生成案例 PDF" disabled={!enabled} tone="secondary" /><Link href={`/admin/cases/${item.id}`}>编辑</Link></article>)}</div>
