...
export function GitHubPagesAdmin() {
  ...
  const persistence = useMemo<AdminPersistence>(() => ({
    saveCase: async (item, isEdit) => { if (!data) throw new Error("内容未加载"); const exists = data.cases.some((entry) => entry.id !== item.id && entry.name === item.name); if (exists) throw new Error("案例名称已存在，请使用唯一名称。"); const previous = data.cases.find((entry) => entry.id === item.id); const cases = isEdit ? data.cases.map((entry) => entry.id === item.id ? item : entry) : [...data.cases, item]; let defaultOrder = data.defaultOrder.filter((entry) => entry !== item.id); let photographyCaseOrder = data.photographyCaseOrder.filter((entry) => entry !== item.id); if (item.business === "photography") photographyCaseOrder = [...photographyCaseOrder, item.id]; else defaultOrder = [...defaultOrder, item.id]; if (previous?.business === item.business) { defaultOrder = item.business === "branding" ? data.defaultOrder : defaultOrder; photographyCaseOrder = item.business === "photography" ? data.photographyCaseOrder : photographyCaseOrder; } await save({ ...data, cases, defaultOrder, photographyCaseOrder }, `${isEdit ? "Update" : "Add"} portfolio case: ${item.name}`); },
    deleteCase: async (item) => { if (!data) throw new Error("内容未加载"); await save({ ...data, cases: data.cases.filter((entry) => entry.id !== item.id), defaultOrder: data.defaultOrder.filter((entry) => entry !== item.id), photographyCaseOrder: data.photographyCaseOrder.filter((entry) => entry !== item.id) }, `Delete portfolio case: ${item.name}`); },
