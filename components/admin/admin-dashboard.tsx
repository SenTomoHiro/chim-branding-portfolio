...
function OrderSection({ title, description, type, cases, order, onOrder, onToggle, onRemove, persistence }: { title: string; description: string; type: "branding" | "photography"; cases: PortfolioCase[]; order: string[]; onOrder: (order: string[]) => void; onToggle: (item: PortfolioCase) => void; onRemove: (item: PortfolioCase) => void; persistence: AdminPersistence }) {
  ...
  return <section className="adminSection" data-order-type={type}>
    ...
    <div className="adminCaseList">{ordered.map((item, index) => <article key={item.id} data-case-id={item.id} onDragOver={(event) => { if (draggedId) event.preventDefault(); }} onDrop={() => { const from = ordered.findIndex((entry) => entry.id === draggedId); const to = ordered.findIndex((entry) => entry.id === item.id); if (from >= 0 && to >= 0 && from !== to) onOrder(move(ordered.map((entry) => entry.id), from, to)); setDraggedId(null); }}>
      <span className="dragHandle" draggable onDragStart={(event) => { setDraggedId(item.id); event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", item.id); }} onDragEnd={() => setDraggedId(null)} aria-label={`拖动 ${caseFullTitle(item)}`} role="button" tabIndex={0}>⠿</span><div className="adminThumb"><img src={contentMediaUrl(getCaseCover(item)!.src)} alt="" draggable={false} /></div><div className="adminCaseName"><strong>{caseFullTitle(item)}</strong><span>{formatCaseMetadata(item, true)}</span></div><button className={`status ${item.published ? "published" : ""}`} onClick={() => onToggle(item)}>{item.published ? "已发布" : "草稿"}</button><div className="mobileReorder"><button disabled={index === 0} onClick={() => onOrder(move(ordered.map((entry) => entry.id), index, index - 1))}>↑</button><button disabled={index === ordered.length - 1} onClick={() => onOrder(move(ordered.map((entry) => entry.id), index, index + 1))}>↓</button></div><Link href={`/admin/cases/?id=${encodeURIComponent(item.id)}`}>编辑</Link><button className="dangerText" onClick={() => onRemove(item)}>删除</button>
    </article>)}</div>
  </section>;
}
