...
function OrderSection({ title, description, type, cases, order, onOrder, onToggle, onRemove, persistence }: { title: string; description: string; type: "branding" | "photography"; cases: PortfolioCase[]; order: string[]; onOrder: (order: string[]) => void; onToggle: (item: PortfolioCase) => void; onRemove: (item: PortfolioCase) => void; persistence: AdminPersistence }) {
  ...
  return <section className="adminSection" data-order-type={type}><div className="sectionHeading"><div><h2>{title}</h2><p>{description}</p></div><div><button className="primaryButton" disabled={pending || !dirty} onClick={saveOrder}>{pending ? "保存中…" : "保存排序"}</button>{dirty && <span className="saveMessage" role="status">排序有未保存修改</span>}{message && <span className="saveMessage" role="status">{message}</span>}</div></div><div className="adminCaseList">{ordered.map((item, index) => <article key={item.id} draggable onDragStart={() => setDragged(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragged !== null) onOrder(move(ordered.map((entry) => entry.id), dragged, index)); setDragged(null); }}><span className="dragHandle">⠿</span><div className="adminThumb"><Image src={assetPath(item.cover)} alt="" fill sizes="100px" /></div><div className="adminCaseName"><strong>{caseFullTitle(item)}</strong><span>{formatCaseMetadata(item, true)}</span></div><button className={`status ${item.published ? "published" : ""}`} onClick={() => onToggle(item)}>{item.published ? "已发布" : "草稿"}</button><div className="mobileReorder"><button disabled={index === 0} onClick={() => onOrder(move(ordered.map((entry) => entry.id), index, index - 1))}>↑</button><button disabled={index === ordered.length - 1} onClick={() => onOrder(move(ordered.map((entry) => entry.id), index, index + 1))}>↓</button></div><Link href={`/admin/cases/${item.id}`}>编辑</Link><button className="dangerText" onClick={() => onRemove(item)}>删除</button></article>)}</div></section>;
}

export function AdminDashboard({ initial, persistence = localPersistence, localPdfEnabled = false }: { initial: ContentData; persistence?: AdminPersistence; localPdfEnabled?: boolean }) {
  ...
  async function toggle(item: PortfolioCase) { try { const next = { ...item, published: !item.published }; if (next.published && next.includeInPortfolioPdf && !next.portfolioPdfImageIds.length) next.portfolioPdfImageIds = createInitialPortfolioPdfSelection(next); await persistence.saveCase(next, true); setData({ ...data, cases: data.cases.map((entry) => entry.id === item.id ? next : entry) }); } catch {} }
