...
import { PDF_COVER_REF, PDF_HERO_REF } from "@/lib/pdf-portfolio";
...
import { PdfGenerateAction } from "./pdf-generation-panel";
...
export function CaseForm({ initial, persistence = localPersistence, localPdfEnabled = false }: { initial?: PortfolioCase; persistence?: AdminPersistence; localPdfEnabled?: boolean }) {
  ...
  const togglePdfImage = (id: string) => setItem((current) => ({ ...current, portfolioPdfImageIds: current.portfolioPdfImageIds.includes(id) ? current.portfolioPdfImageIds.filter((entry) => entry !== id) : [...current.portfolioPdfImageIds, id] }));
  const movePdfImage = (id: string, offset: number) => setItem((current) => {
    const portfolioPdfImageIds = [...current.portfolioPdfImageIds]; const from = portfolioPdfImageIds.indexOf(id); const to = from + offset;
    if (from < 0 || to < 0 || to >= portfolioPdfImageIds.length) return current;
    [portfolioPdfImageIds[from], portfolioPdfImageIds[to]] = [portfolioPdfImageIds[to], portfolioPdfImageIds[from]];
    return { ...current, portfolioPdfImageIds };
  });
  const pdfChoice = (id: string, label = "合集精选") => {
    const position = item.portfolioPdfImageIds.indexOf(id); const selected = position >= 0;
    return <div className="pdfInlineChoice"><label className="checkLabel"><input type="checkbox" checked={selected} onChange={() => togglePdfImage(id)} /> {label}</label>{selected && <><b>{String(position + 1).padStart(2, "0")}</b><button type="button" aria-label={`${label}上移`} disabled={position === 0} onClick={() => movePdfImage(id, -1)}>↑</button><button type="button" aria-label={`${label}下移`} disabled={position === item.portfolioPdfImageIds.length - 1} onClick={() => movePdfImage(id, 1)}>↓</button></>}</div>;
  };
  ...
  return <main className="caseEditor"><div className="editorHeading"><div><p>Cases / {isPersisted ? "Edit" : "New"}</p><h1>{isPersisted ? "编辑案例" : "新建案例"}</h1></div><div className="saveRow"><span className="saveMessage" role="status">{feedback}</span><Link href="/admin" onClick={leave}>返回后台</Link><button form="case-form" className="primaryButton" disabled={pending || !dirty}>{pending ? "保存中…" : "保存案例"}</button></div></div><form id="case-form" onSubmit={submit}>
    ...
    <section className="formSection chapterEditor"><div className="sectionHeading"><div><h2>正文媒体</h2><p>正文媒体是唯一列表；图片可直接勾选进入合集，并独立调整精选顺序。</p></div><div className="bodyMediaActions"><label className="checkLabel pdfMembership"><input type="checkbox" checked={item.includeInPortfolioPdf} onChange={(event) => set("includeInPortfolioPdf", event.target.checked)} /> 加入合集 PDF</label>{localPdfEnabled && isPersisted && <PdfGenerateAction target={`case:${item.id}`} label="生成案例 PDF" disabled={dirty || pending} hint={dirty ? "请先保存" : undefined} />}<button type="button" className="secondaryButton" disabled={!item.bodyAssets.length} onClick={() => openChapterDraft()}>＋ 添加章节</button><label className="primaryButton">{hasChapters ? "上传未分章节媒体" : "上传媒体"}<input type="file" accept="image/*,video/mp4,video/webm" onChange={(event) => addBody(event.target.files?.[0])} /></label></div></div>
      {(item.hero || item.cover) && <div className="pdfSourceReferences" aria-label="案例主图合集精选">{item.hero && <article><div className="assetPreview"><Image src={assetPath(item.hero)} alt="案例详情页首图" fill sizes="120px" /></div><div><strong>案例详情页首图</strong><span>Hero</span></div>{pdfChoice(PDF_HERO_REF, "首图合集精选")}</article>}{item.cover && <article><div className="assetPreview"><Image src={assetPath(item.cover)} alt="案例列表封面" fill sizes="120px" /></div><div><strong>案例列表封面</strong><span>Cover</span></div>{pdfChoice(PDF_COVER_REF, "封面合集精选")}</article>}</div>}
