...
export function CaseForm({ initial }: { initial?: PortfolioCase }) {
  ...
  async function addBody(file?: File) {
    ...
    if (!response.ok) { alert(body.error); return; }
    setItem({ ...item, bodyAssets: [...item.bodyAssets, { id: `upload-${Date.now()}`, type: body.type, src: body.src, layout: "full" }] });
  }
  ...
  return <main className="caseEditor"><div className="editorHeading"><div><p>Cases / {isEdit ? "Edit" : "New"}</p><h1>{isEdit ? "编辑案例" : "新建案例"}</h1></div><button form="case-form" className="primaryButton" disabled={pending}>{pending ? "保存中…" : "保存案例"}</button></div><form id="case-form" onSubmit={submit}>
    ...
    <section className="formSection"><h2>封面与 Hero</h2><div className="mediaInputs"><MediaInput label="Cover" value={item.cover} onChange={(value, width, height) => setItem({ ...item, cover: value, coverWidth: width || item.coverWidth, coverHeight: height || item.coverHeight })} /><MediaInput label="Hero" value={item.hero} onChange={(value) => set("hero", value)} /></div></section>
