...
const empty: PortfolioCase = { id: `C${Date.now()}`, brandName: "", projectName: "", intro: "", business: "branding", categories: [], primaryIndustry: "", cover: "", coverWidth: 1400, coverHeight: 1050, hero: "", bodyAssets: [], published: false, includeInPortfolioPdf: true, portfolioPdfHeroSelected: false, portfolioPdfCoverSelected: false };
...
export function CaseForm({ initial, persistence = localPersistence, localPdfEnabled = false }: { initial?: PortfolioCase; persistence?: AdminPersistence; localPdfEnabled?: boolean }) {
  ...
  const pdfChoice = (selected: boolean, onChange: (selected: boolean) => void, label = "合集精选") => <div className="pdfInlineChoice"><label className="checkLabel"><input type="checkbox" checked={selected} onChange={(event) => onChange(event.target.checked)} /> {label}</label></div>;
  ...
  return <main className="caseEditor"><div className="editorHeading"><div><p>Cases / {isPersisted ? "Edit" : "New"}</p><h1>{isPersisted ? "编辑案例" : "新建案例"}</h1></div><div className="saveRow"><span className="saveMessage" role="status">{feedback}</span><Link href="/admin" onClick={leave}>返回后台</Link><button form="case-form" className="primaryButton" disabled={pending || !dirty}>{pending ? "保存中…" : "保存案例"}</button></div></div><form id="case-form" onSubmit={submit}>
    ...
    <section className="formSection chapterEditor"><div className="sectionHeading"><div><h2>正文媒体</h2><p>勾选决定是否进入合集；调整正文顺序会同步网站与全部 PDF。</p></div><div className="bodyMediaActions"><label className="checkLabel pdfMembership"><input type="checkbox" checked={item.includeInPortfolioPdf} onChange={(event) => set("includeInPortfolioPdf", event.target.checked)} /> 加入合集 PDF</label>{localPdfEnabled && isPersisted && <PdfGenerateAction target={`case:${item.id}`} label="生成案例 PDF" disabled={dirty || pending} hint={dirty ? "请先保存" : undefined} />}<button type="button" className="secondaryButton" disabled={!item.bodyAssets.length} onClick={() => openChapterDraft()}>＋ 添加章节</button><label className="primaryButton">{hasChapters ? "上传未分章节媒体" : "上传媒体"}<input type="file" accept="image/*,video/mp4,video/webm" onChange={(event) => addBody(event.target.files?.[0])} /></label></div></div>
      {(item.hero || item.cover) && <div className="pdfSourceReferences" aria-label="案例主图合集精选">{item.hero && <article><div className="assetPreview"><Image src={assetPath(item.hero)} alt="案例详情页首图" fill sizes="120px" /></div><div><strong>案例详情页首图</strong><span>Hero · 固定在精选首位</span></div>{pdfChoice(item.portfolioPdfHeroSelected, (selected) => set("portfolioPdfHeroSelected", selected), "首图合集精选")}</article>}{item.cover && <article><div className="assetPreview"><Image src={assetPath(item.cover)} alt="案例列表封面" fill sizes="120px" /></div><div><strong>案例列表封面</strong><span>Cover · 固定在 Hero 之后</span></div>{pdfChoice(item.portfolioPdfCoverSelected, (selected) => set("portfolioPdfCoverSelected", selected), "封面合集精选")}</article>}</div>}
