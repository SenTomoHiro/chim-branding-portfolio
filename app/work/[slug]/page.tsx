...
export default async function WorkPage({ params }: { params: Promise<{ slug: string }> }) {
  const route = await params; const slug = decodeSlug(route.slug); const data = await readContent(); const ordered = sortPublishedCases(data.cases, data.defaultOrder); const index = ordered.findIndex((entry) => entry.slug === slug); if (index < 0) notFound(); const item = ordered[index]; const next = ordered[(index + 1) % ordered.length];
