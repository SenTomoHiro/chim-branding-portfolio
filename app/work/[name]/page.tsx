...
export default async function WorkPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  return <main className="workPage"><SiteHeader business={current.business} detail /><BackToTopButton /><ViewTransition name={`case-image-${item.id}`} share="case-morph" default="none"><div className="workHero"><Image src={assetPath(item.hero)} alt={`${item.name} 项目主视觉`} fill priority sizes="100vw" /></div></ViewTransition><section className="workIntro"><div><p>{formatCaseMetadata(item)}</p><h1>{item.name}</h1></div><p>{item.intro}</p></section><section className="mediaFlow">{item.bodyAssets.map((media) => <RevealMedia key={media.id} media={media} caseName={item.name} />)}</section><NextCaseLink current={item} next={next} /></main>;
}
