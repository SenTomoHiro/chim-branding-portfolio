...
export async function PUT(request: Request) {
  ...
  const data = await readContent(); const body = await request.json(); const photography = body.type === "photography"; const valid = new Set(data.cases.filter((item) => photography ? item.designPrimary === "Photography" : item.designPrimary !== "Photography").map((item) => item.id));
