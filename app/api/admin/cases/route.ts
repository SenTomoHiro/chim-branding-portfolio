...
export async function POST(request: Request) {
  ...
  try { const data = await readContent(); const item = parseCase(await request.json()); assertUniqueName(data, item); data.cases.push(item); if (item.business === "photography") data.photographyCaseOrder.push(item.id); else data.defaultOrder.push(item.id); await writeContent(data); return NextResponse.json(item, { status: 201 }); }
