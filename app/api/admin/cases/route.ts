...
export async function POST(request: Request) {
  ...
  try { const data = await readContent(); const item = parseCase(await request.json()); assertUniqueSlug(data, item); data.cases.push(item); data.defaultOrder.push(item.id); await writeContent(data); return NextResponse.json(item, { status: 201 }); }
