...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try { const previous = data.cases[index]; const item = parseCase(await request.json(), previous); assertUniqueSlug(data, item); data.cases[index] = item; if (previous.designPrimary !== item.designPrimary) { data.defaultOrder = data.defaultOrder.filter((caseId) => caseId !== id); data.photographyCaseOrder = data.photographyCaseOrder.filter((caseId) => caseId !== id); if (item.designPrimary === "Photography") data.photographyCaseOrder.push(id); else data.defaultOrder.push(id); } await writeContent(data); return NextResponse.json(item); }
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  data.cases = data.cases.filter((item) => item.id !== id); data.defaultOrder = data.defaultOrder.filter((caseId) => caseId !== id); data.photographyCaseOrder = data.photographyCaseOrder.filter((caseId) => caseId !== id); data.versions = data.versions.map((version) => ({ ...version, priorityCaseIds: version.priorityCaseIds.filter((caseId) => caseId !== id) }));
