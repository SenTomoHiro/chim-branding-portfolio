...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try { const item = parseCase(await request.json(), data.cases[index]); assertUniqueSlug(data, item); data.cases[index] = item; await writeContent(data); return NextResponse.json(item); }
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  data.cases = data.cases.filter((item) => item.id !== id); data.defaultOrder = data.defaultOrder.filter((caseId) => caseId !== id); data.versions = data.versions.map((version) => ({ ...version, priorityCaseIds: version.priorityCaseIds.filter((caseId) => caseId !== id) }));
