...
test("Pages admin keeps its list readable and navigates to generated editor routes", async ({ page }) => {
  ...
  await expect(page.locator(".githubPagesAdmin .adminCaseList article")).toHaveCount(content.cases.length);
  expect(await page.locator(".githubPagesAdmin .adminCaseList article").first().locator(".adminCaseName").evaluate((cell) => cell.getBoundingClientRect().width)).toBeGreaterThan(180);
  expect(await page.locator(".githubPagesAdmin .adminCaseList article").first().getByRole("link", { name: "编辑" }).evaluate((link) => link.getBoundingClientRect().width)).toBeLessThan(100);
